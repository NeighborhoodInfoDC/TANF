...
* Version:  SAS 8.12
* Environment:  Windows
* Description:  This program reads in raw 1999 TANF data, formats it, aggregates, and merges. Figures out which cases have PIs and which don't, and figures out
...
%include "D:\METRO\JFenders\Working TANF\1999\DCtanfform.sas";
libname library "d:\Metro\JFenders\Working TANF\formats";
libname DC_TANF "d:\Metro\JFenders\Working TANF\1999" ;
libname convert "d:\Metro\JFenders\Working TANF\1999\sasprograms";
...
title "DC TANF 1999 (July) data, DCtanfdata.sas" ;


*NEED TO UPDATE THE FOLLOWING VARIABLES -- SHOULD BE ONLY SECTION NECESSARY TO UPDATE;

%let rawfolder = d:\Metro\JFenders\Working TANF\TANF\1999\raw1999data\;
%let client = client data 199907.txt;
%let case = case data 199907.txt;
%let yearmonth =199907 ;
%let abyrmonth = 9907;

*Change baseage -- make it the first of January or July, and the year;
%let baseage ='01JUL99'D;
%let agelabel= Age as of July 1, 1999;
%let adultagelab = Age of adult as of 7/1/99;
%let childagelab = Age of child as of 7/1/99 ;


*read in raw client data;

data DC_TANF.TANFclie&yearmonth.;
infile 'd:\Metro\JFenders\Working TANF\1999\raw1999data\clnt0799.txt' lrecl=35;
...
infile 'd:\Metro\JFenders\Working TANF\1999\raw1999data\case0799.txt' lrecl=12;
...
proc summary data =DC_Tanf.Tanfclie&yearmonth. (where=(FULPART_&abyrmonth.=1));
	...
	by caseid ;
	output out=aggr1999clie sum= ;
	run;
	
proc print data =aggr1999clie (obs=20);
	title "Aggregate of different aged children, PI and Full Participants by case id" ;
	

proc freq data=aggr1999clie ;
...
proc print data = DC_TANF.TANF&yearmonth._80tracts  (obs=10);
	title "1999 (July) TANF Data in 1980 tracts" ;
run;
