...
title "DC TANF 2003 (July) data, DCtanfdata.sas" ;


*NEED TO UPDATE THE FOLLOWING VARIABLES -- SHOULD BE ONLY SECTION NECESSARY TO UPDATE;

%let rawfolder = d:\Metro\JFenders\Working TANF\2003\raw2003data\;
%let client = client data 200307.txt;
%let case = case data 200307.txt;
%let yearmonth = 200307 ;
%let abyrmonth = 0307;

*Change baseage -- make it the first of January or July, and the year;
%let baseage ='01JUL03'D;
%let agelabel= Age as of July 1, 2003;
%let adultagelab = Age of adult as of 7/1/03;
%let childagelab = Age of child as of 7/1/03 ;


*read in raw client data;

data DC_TANF.TANFclie&yearmonth.;
infile 'd:\Metro\JFenders\Working TANF\2003\dhsa0218jul2003.txt' truncover lrecl=35;
...
infile 'd:\Metro\JFenders\Working TANF\2003\dhsa0216jul2003.txt' truncover lrecl=12;
...
proc summary data =DC_Tanf.Tanfclie&yearmonth.  (where=(FULPART_&abyrmonth.=1));
	...
	by caseid ;
	output out=aggr2003clie sum= ;
	run;
	
proc print data =aggr2003clie (obs=20);
	title "Aggregate of different aged children, PI and Full Participants by case id" ;
	

proc freq data=aggr2003clie ;
...
proc print data = DC_TANF.TANF&yearmonth._80tracts  (obs=10);
	title "2003 (July) TANF Data in 1980 tracts" ;
run;
