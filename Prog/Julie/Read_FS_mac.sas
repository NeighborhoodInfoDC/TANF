...
%macro Read_FS_mac( rawfolder =, client =, case =,  year =,  month =);


*read in raw client data;

data TANF.FS_&year._&month._client (label="Food Stamp cases, DC, &month/&year, client-level data");
...
*Made dummy variables for children age ranges ;
	...
	label Teen = "Teen" ;
	format Child dyesno. Fulpart dyesno. Unborn dyesno. Infant dyesno.
			Todd dyesno. Preteen dyesno. Teen dyesno.;
...
of household);
	...
	format PI dyesno.;
run;        
proc sort data= Tanf.FS_&year._&month._client  ;
	by caseid clientid;
	run;
...
data TANF.FS_&year._&month._case (label="Food Stamp cases, DC, &month/&year, case-level data");
...
proc sort data= Tanf.FS_&year._&month._case  ;
	by caseid;
	run;
*Julie, check to see if syntax is correct;
data Tanf.FS_&Year._&month (label="Food Stamp client case data &month/&year, DC" drop=dob clientid caseid);
merge TANF.FS_&year._&month._case (in=case) TANF.FS_&year._&month._client (in=client);
	by caseid;
...
%File_info( data=Tanf.FS_&year._&month._case, stats=, freqvars=language )
%File_info( data=Tanf.FS_&year._&month._client, freqvars=educat ethnic 
	partcode relcode sex AdultAge ChildAge Fulpart Unborn Infant Todd Preteen 
	Teen Child Adult PI)
%File_info( data=Tanf.FS_&Year._&month )
%mend Read_FS_mac;
