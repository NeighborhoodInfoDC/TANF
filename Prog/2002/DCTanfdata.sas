...
* Version:  SAS 8.12
* Environment:  Windows
* Description:  This program reads in raw 2002 TANF data, formats it, aggregates, and merges. Figures out which cases have PIs and which don't, and figures out
...
%include "D:\METRO\JFenders\Working TANF\2003\DCtanfform.sas";
libname library "d:\Metro\JFenders\Working TANF\formats";

libname DC_TANF "d:\Metro\JFenders\Working TANF\2003" ;
libname convert "d:\Metro\JFenders\Working TANF\2003\sasprograms";
...
title "DC TANF 2002 (July) data, DCtanfdata.sas" ;


*NEED TO UPDATE THE FOLLOWING VARIABLES -- SHOULD BE ONLY SECTION NECESSARY TO UPDATE;

%let rawfolder = d:\Metro\JFenders\Working TANF\TANF\2002\raw2002data\;
%let client = client data 200207.txt;
%let case = case data 200207.txt;
%let yearmonth = 200207 ;
%let abyrmonth = 0207;

*Change baseage -- make it the first of January or July, and the year;
%let baseage ='01JUL02'D;
%let agelabel= Age as of July 1, 2002;
%let adultagelab = Age of adult as of 7/1/02;
%let childagelab = Age of child as of 7/1/02 ;


*read in raw client data;

data DC_TANF.TANFclie&yearmonth.;
infile 'd:\Metro\JFenders\Working TANF\2002\raw2002data\client data 200207.txt' lrecl=35;
...
infile 'd:\Metro\JFenders\Working TANF\2002\raw2002data\case data 200207.txt' lrecl=12;
...
age as of July 1 of the year of the data. In this case, it"s 2002. There are missing
...
proc freq data=DC_Tanf.Tanfclie&yearmonth. ;
	...
	proc contents data=dc_tanf.tanfclie200207;
	run;

*****************************START HERE FOR 2002 DATA;
...
proc summary data =DC_Tanf.Tanfclie&yearmonth.  (where=(FULPART_&abyrmonth.=1));
	...
	by caseid ;
	output out=aggr2002clie sum= ;
	run;
	
proc print data =aggr2002clie (obs=20);
	title "Aggregate of different aged children, PI and Full Participants by case id" ;
	

proc freq data=aggr2002clie ;
...
proc summary data=tract2;
   by geo80;
   var hh_&abyrmonth. FULPART_&abyrmonth. PI_&abyrmonth. UNBORN_&abyrmonth. INFANT_&abyrmonth. TODD_&abyrmonth. PRETEEN_&abyrmonth. TEEN_&abyrmonth. No_kids_&abyrmonth. ADULT_&abyrmonth.;
...
set DC_TANF.TANF&yearmonth._80tracts ;
	...
	*insert #3;
	array miss0 {*} hh_&abyrmonth. PI_&abyrmonth. FULPART_&abyrmonth. No_kids_&abyrmonth. ADULT_&abyrmonth.;
	do i = 1 to dim(miss0) ;
	if miss0{i} in (.) then miss0{i} =0 ;
	end;

	label HH_&abyrmonth. = "TANF households";
...
proc print data = DC_TANF.TANF&yearmonth._80tracts  (obs=10);
	title "2002 (July) TANF Data in 1980 tracts" ;
run;
